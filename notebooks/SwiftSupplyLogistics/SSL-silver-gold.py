...
#save delta tables as parquet for analytics in Synapse warehouse

spark.read.format("delta").table("ssl_db.supplychain_snapshot").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/supplychain_snapshot_parquet")
spark.read.format("delta").table("ssl_db.alerts_shipments").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/alerts_shipments_parquet")
spark.read.format("delta").table("ssl_db.fact_sales_daily").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/fact_sales_daily_parquet")
spark.read.format("delta").table("ssl_db.fact_shipments").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/fact_shipments_parquet")
spark.read.format("delta").table("ssl_db.fact_inventory_daily").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/fact_inventory_daily_parquet")
spark.read.format("delta").table("ssl_db.dim_product").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/dim_product_parquet")
spark.read.format("delta").table("ssl_db.dim_warehouse").repartition(1).write.mode("overwrite").format("parquet").save(f"{base_path_gold}/dim_warehouse_parquet")
...
ack_df = spark.createDataFrame([("acknowledged",)], ["status"])
ack_df.coalesce(1).write.mode("overwrite").format("text").save(f"{base_path_gold}/_success")
print("Acknowledgement file created successfully.")
